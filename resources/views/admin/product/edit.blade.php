        ...
        <div class="card">
            ...
            <div class="card-body card-block">
                <form action="{{ route('product.update', $product->id) }}" method="post" enctype="multipart/form-data">
                    ...
                    <div class="form-group mb-2">
                        <label for="subSubCategory" class="form-control-label">Sub Sub Category Name</label>
                        <select name="sub_sub_category_id" id="subSubCategory" class="form-control select2" data-toggle="select2" data-placeholder="Select a sub category">
                            <option value="{{ $product->subSubCategory->id }}">{{ $product->subSubCategory->name }}</option>
                            @foreach($SubSubCategories as $item)
                            <option value="{{ $item->id }}" {{ $item->id == $product->sub_sub_category_id ? 'selected' : '' }}>{{ $item->name }}</option>
                        @endforeach
                        </select>
                    </div>
                    <div class="form-group mb-2">
                        ...
                        <select name="enterprise_id" id="enterprise_id" class="form-control select2" data-toggle="select2" data-placeholder="Select Entreprenerus">
                            @foreach($enterprises as $enterprise)
                                <option value="{{ $enterprise->id }}" {{ $enterprise->id == $product->enterprise_id ? 'selected' : '' }}>{{ $enterprise->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="form-group mb-2">
                    ...
                    <div class="form-group mb-2">
                        ...
                        <input type="text" id="discount" name="discount" value="{{ $product->discount }}" class="form-control">
                    </div>
                    <div class="form-group mb-2">
                        <label for="description" class=" form-control-label">Product Discription</label>
